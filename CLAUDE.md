...
**当前关注**: 基于 AUR 驱动 (v2.1) 的测试和 MLO 研究。
...
| **MLO 5GHz+2.4GHz 双链路** | **✅ iperf3 433Mbps, 两band均传数据 (45%/54%)** |
| **MLO 6GHz+2.4GHz 双链路** | **⚠️ 关联成功，数据路径待验证 (需干净测试)** |
| **MLO 三链路发现** | **✅ 6G+5G+2.4G 全部发现 (valid_links=0x7)** |

### MLO 补丁与上游提交

**基础补丁**: `patches/mt6639-wifi-mlo.patch` (6 文件, ~340 行)
- PR: https://github.com/jetm/mediatek-mt7927-dkms/pull/7
- jetm 正在准备内核上游提交，已提供 `Signed-off-by: 张旭涵 <Loong.0x00@gmail.com>`
- 测试结果: https://github.com/openwrt/mt76/issues/927#issuecomment-3998815138

### MLO 修复 (基础补丁 5 处 + 新发现 3 处)

**基础补丁 (patches/mt6639-wifi-mlo.patch, 已提交 PR):**
1. **MLO 能力强制** (`mt7925/init.c`): MT6639 固件不报告 MLO_EN，force `chip_cap |= MT792x_CHIP_CAP_MLO_EN`
2. **RX WCID NULL 修复** (`mt792x_mac.c:149`): BSS self-STA WCID (sta=0) 返回 `wcid->def_wcid` 而非 NULL
3. **omac_idx 唯一化** (`mt7925/main.c:405`): MT6639 MLD 每链路独立 omac_idx
4. **band_idx 修正** (`mt7925/main.c:416`): 从 link_conf 获取频段而非共享 phy chandef
5. **ROC 条件修复** (`mt7925/main.c:1095`): `band != secondary_band` 覆盖所有跨频段组合

**新发现修复 (2026-03-06, 尚未提交):**
6. **ROC grant timer 修复** (`mt7925/mcu.c:380`): 固件 ROC grant 回 reqtype=10 (MLSR_AG)，
   不是 JOIN(0)。条件改为 `reqtype != MT7925_ROC_REQ_ROC`，对所有 MLO grant 设 24h timer。
   **没有此修复时 secondary link 5 秒后被 abort，数据全走 primary 一个 band。**
7. **cfg80211 BSS 查找修复** (out-of-tree `cfg80211.ko`): `nl80211_assoc_bss()` 跳过
   `USE_FOR_MLD_LINK` 要求，允许无 RNR 的 partner BSS 关联。
8. **5GHz↔6GHz 链路排除** (`mac80211.c:mt76_select_links()`): MT6639 同射频不能 STR。

### 6GHz MLO 注意事项

- AP MLO 6GHz 频道: **6295 MHz (ch53)**，不是之前错记的 6615
- **需要清除 NO_IR + DISABLED** 才能扫到 6GHz (`init.c` 中 `iw reg set US` + 清标志)
- 之前记录的"不需要清除 NO_IR"是错误的——ML element 发现链路但 wpa_supplicant 的 ML probe 无法 resolve
- 6GHz 做 primary link 时数据路径可能有问题，**需要干净环境重新验证**
- 之前"6G+2.4G ping 10/10"是在开着 NO_IR 清除时测的，后来关了就再没验证过数据
...
**PHY 速率**: TX ~1297 MBit/s / RX ~1361 MBit/s (EHT MCS 12/13, NSS 2, 80MHz)

### 当前 DKMS 源码状态 (重要!)

`/usr/src/mediatek-mt7927-2.1/mt76/` 中的文件已被多次手动 sed 修改，状态不干净。
**下次操作前必须**: 重新安装 AUR 包恢复原始源码，然后只应用 `patches/mt6639-wifi-mlo.patch` +
ROC timer 修复 + 5G↔6G 排除 + NO_IR 清除（如需测 6GHz）。

### AUR 驱动 MLO 代码分析
- 源码: `/usr/src/mediatek-mt7927-2.1/mt76/`
- MLO 回调已实现: `change_vif_links`, `change_sta_links`, `link_info_changed`
- MCU TLV 已实现: STA_REC_MLD (0xE0), EHT_MLD (0xFB), BSS_MLD (0x1A)
- STR vs EMLSR 检测已实现, 双频 ROC 已实现
- **关键门控**: `MT792x_CHIP_CAP_MLO_EN` 标志 (main.c:286)
- marcin-fm 正在开发 STR MLO 补丁，也在 review 我们的 PR
