...
**MT6639 硬件规格**:
- WiFi 7 (802.11be), 三频 2.4GHz / 5GHz / 6GHz
- 2x2 MIMO, EHT 320MHz, 4K-QAM (MCS12/13)
- PHY 速率: RX 5187 Mbps / TX 4803 Mbps (320MHz), RX 1361 / TX 1297 Mbps (80MHz)
- **2 个射频 (RF)**: RF0 = 2.4GHz, RF1 = 5GHz/6GHz (共享)
- **最多同时 2 条链路** (STR): 2.4G+5G 或 2.4G+6G
- 5GHz+6GHz 不可能同时 (同一射频 RF1, band_idx=1)
- MLO 架构: Single-MAC, 固件 UMAC 自动做 MPDU 跨链路分发
- DBDC (Dual-Band Dual-Concurrent) 模式, 非 EMLSR
...
| **6GHz 连接 (WPA3-SAE)** | **✅ 20/80/160/320MHz 全通 (v3 补丁修复 320MHz)** |
| WiFi 7 EHT 320MHz 4K-QAM | **✅ PHY 4803 Mbps EHT-MCS11, iperf3 841 Mbps** |
| **MLO 5GHz+2.4GHz STR** | **✅ iperf3 763Mbps/30s 稳定** |
| **MLO 三链路发现** | **✅ 6G+5G+2.4G 全部发现 (valid_links=0x7), active 选 5G+2.4G** |
| **MLO 三链路 (6G+5G+2.4G)** | **✅ 741 Mbps/30s, PHY 4803 Mbps 320MHz** |

### MLO 补丁

**当前补丁**: `patches/mt6639-wifi-mlo-v3.patch` (4 文件, 95 行) — 适配 v2.1-22
- PR: https://github.com/jetm/mediatek-mt7927-dkms/pull/22
- jetm 正在准备内核上游提交，`Signed-off-by: 张旭涵 <Loong.0x00@gmail.com>`
- **cfg80211 补丁不需要** — v2.1-22 的 band_idx 修复已解决 BSS 查找问题

### MLO v3 补丁修复 (4 处, 适配 v2.1-22)

v2.1-22 已包含 band_idx 修复，以下为 v2.1-22 之上的增量修改:

1. **MLO 能力强制** (`init.c`): `chip_cap |= MT792x_CHIP_CAP_MLO_EN`
2. **RX WCID 修复** (`mt792x_mac.c`): BSS self-STA 返回 `def_wcid` 而非 NULL
3. **320MHz BSS_RLM** (`mcu.c`): 添加 `NL80211_CHAN_WIDTH_320` case — **修复 320MHz 数据不通**
4. **5GHz↔6GHz 排除** (`mac80211.c`): MT6639 同射频不能 STR

v2 中的 omac_idx、ROC 条件、roc_iter MLD 遍历、link_num=3 在 v2.1-22 上不需要。

### 320MHz BSS_RLM 根因 (v3 新修复)

`mt7925_mcu_bss_rlm_tlv()` 的 switch 缺少 `NL80211_CHAN_WIDTH_320` case，
320MHz 时 fallback 到 `default: bw = CMD_CBW_20MHZ`，固件按 20MHz 配置 RX 射频，
无法解码 AP 发的 320MHz 帧。AP 端表现为 retry exhausted 43%，TX rate 降到 MCS 0。

### 压力测试结果 (2026-03-06)

```
6GHz 320MHz: iperf3 -t 30 -P 8 → 841 Mbps, 2.94 GB, PHY 4803 Mbps EHT-MCS11
MLO 5G+2.4G: iperf3 -t 30 -P 8 → 763 Mbps, 2.66 GB, 30s 稳定
MLO 3-link:  iperf3 -t 30 -P 8 → 741 Mbps, 2.59 GB, PHY 4803 Mbps 320MHz
```

### 6GHz 数据路径 — ✅ 正常工作 (之前误判)

**6GHz 数据路径完全正常！** 之前所有 "6GHz 不通" 的结论都是路由/IP 配置错误导致的误判。

**根因**: wlp9s0 和 enp12s0 (有线) 都在 10.0.0.0/24 子网，`ping -I wlp9s0` 选了
enp12s0 的源地址 10.0.0.2，回复走有线而非 WiFi。`iw dev link` 显示 `6.0 MBit/s`
只是因为没数据流量、rate adaptation 未启动 — 一旦有真实流量立即跳到 EHT-MCS 13。

**正确测试方法**: 手动配 IP + 指定源地址
```bash
sudo ip addr add 10.0.0.10/24 dev wlp9s0
ping -I 10.0.0.10 10.0.0.1        # 不要用 -I wlp9s0
iperf3 -c 10.0.0.1 -B 10.0.0.10
```

**6GHz 带宽测试结果** (2026-03-06, ASUS RT-BE92U, fw 20260123):

| 带宽 | PHY TX | iperf3 | Retrans | 状态 |
|------|--------|--------|---------|------|
| 20MHz (ch5) | 344 Mbps EHT-MCS13 | 184 Mbps | 0 | ✅ |
| 80MHz (ch1) | 1441 Mbps EHT-MCS13 | 412 Mbps | 20 | ✅ |
| 160MHz (ch69) | 2402 Mbps EHT-MCS11 | 688 Mbps | 2 | ✅ 最佳 |
| **320MHz (ch53)** | **4803 Mbps EHT-MCS11** | **841 Mbps** | 3414 | **✅ v3 补丁修复** |

- 320MHz 根因: `bss_rlm_tlv()` 缺少 `NL80211_CHAN_WIDTH_320` case, 固件 RX 配置为 20MHz
- marcin-fm (BPI-R4, 20MHz): 262 Mbps — 量级一致

**marcin-fm 对 MLO 补丁的意见** (jetm#15, 2026-03-05):
- 建议**不合并 MLO 补丁**，稳定性不够
- 建议只保留 band_idx 补丁 (单链路降级连接 MLO 网络)
