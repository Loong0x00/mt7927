...
| RX Ring 4 DMA 投递 | ✅ S39 BNRCFR 修复 |
| RX 帧处理 → mac80211 | ✅ S40 四修复 (见下方) |
| **802.11 Authentication** | **✅ S40 Auth-2 收到, status=0** |
| **802.11 Association** | **✅ S40 Assoc Response 收到** |
| STA_REC STATE=2 post-assoc | ✅ S41 修复 |
| **EAPOL 4-way Handshake** | **❌ 阻塞: AP 不发 EAPOL Key 1/4** |

### 当前阻塞: EAPOL 4-way Handshake 超时

详见 `docs/debug/eapol_ack_problem.md`

**核心事实**: Auth + Assoc 均成功，但 AP 从不发 EAPOL Key 1/4。
AR9271 空口抓包发现**双向 ACK 均不工作** — 每帧重传 30 次。
AP 可能因未收到 Assoc Response 的 ACK 而不启动 EAPOL。

### S40 关键突破: Auth + Association 成功 (28 session 阻塞解除)

四个修复共同作用:
1. **BssActivateCtrl band_idx = 0xFF** (Windows RE: 继承 DEV_INFO, 不覆盖)
2. **BASIC TLV active = mvif->bss_idx** (=0, 不是 boolean 1)
3. **Full BSS_INFO link_idx = 0** (只有 BssActivateCtrl 写 link_idx)
4. **RFCR DROP_OTHER_UC 清除** (ROC_GRANT handler 中清除 BIT(18))
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `src/mt7927_pci.c` | ~4600 | 初始化、MCU 通信、mac80211 回调、MCU 命令构建 |
| `src/mt7927_pci.h` | ~1970 | 寄存器定义、数据结构、宏 |
| `src/mt7927_mac.c` | ~1480 | TXD 构建 (统一路径)、RXD 解析、RX 分发 |
