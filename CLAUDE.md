...
## AUR 驱动测试状态 (2026-03-04)

| 功能 | AUR v2.1 状态 |
|------|---------------|
| 2.4GHz 连接 (WPA2-PSK) | ✅ TP-LINK + ASUS |
| 5GHz 连接 (WPA2-PSK) | ✅ ASUS RT-BE92U |
| 6GHz 连接 (WPA3-SAE) | ✅ ASUS RT-BE92U (320MHz, EHT) |
| WiFi 7 EHT 320MHz 4K-QAM | ✅ PHY RX 5187 Mbps / TX 4803 Mbps |
| **MLO 单链路 (数据平面)** | **✅ valid_links=0x4, ping 成功** |
| **MLO 双链路 (5GHz+2.4GHz)** | **✅ valid_links=0x6, ping 10/10, avg 2.7ms** |
| **MLO 双链路 (6GHz+2.4GHz)** | **✅ valid_links=0x5, ping 30/30 (6GHz 信号弱时回退)** |

### MLO 修复进展 (2026-03-04)

**已修复 — 双链路 MLO (5GHz + 2.4GHz) 数据平面完全工作**:
- SSID: asus_MLO (ASUS RT-BE92U, 三频 MLO 开启)
- MLD 发现: valid_links=0007 (link0=6G, link1=5G, link2=2.4G)
- 连接: COMPLETED, ap_mld_addr=60:cf:84:fd:2d:aa, wifi_generation=7
- **双链路激活**: Link 1 (5785 MHz) + Link 2 (2412 MHz), STA_MLD link_num=2 valid=0x6
- **Ping 192.168.52.1: 10/10 成功, avg 2.7ms**
- 详见: `docs/debug/mlo_single_link_dataplane_fix.md`, `docs/debug/mlo_dual_link_fix.md`

**关键修复 (5 处)**:
1. **RX WCID NULL 修复** (`mt792x_mac.c:149`): BSS self-STA WCID (sta=0) 返回 `wcid->def_wcid` 而非 NULL
2. **omac_idx 唯一化** (`mt7925/main.c:405`): MT6639 MLD 每链路独立 omac_idx
3. **band_idx 修正** (`mt7925/main.c:416`): 从 link_conf 获取频段而非共享 phy chandef
4. **ROC 条件修复** (`mt7925/main.c:1095`): `band != secondary_band` 覆盖所有跨频段组合
5. **Regulatory domain**: `iw reg set US` + `scan_freq=5785 2412` 确保 5GHz 主动扫描

**PHY 速率**:
- TX: 1296.7 MBit/s EHT-MCS 12 NSS 2 80MHz (初始 OFDM 6Mbps, 速率自适应正常)
- RX: 1361.1 MBit/s EHT-MCS 13 NSS 2 80MHz

**当前局限**:
- 只测试了 5GHz+2.4GHz 双链路, 6GHz 链路未激活 (US regulatory PASSIVE-SCAN)
- mt7925/mt76 原设计为单射频 eMLSR, STR 多射频支持是通过 MT6639 补丁实现
...
- marcin-fm 正在开发 STR MLO 补丁 (尚未发布)
