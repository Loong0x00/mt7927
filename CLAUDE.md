...
| **RX 接收** | **❌ 阻塞: Ring 4 DIDX=0, 固件从不向 Ring 4 投递帧** |

### 当前阻塞: RX Ring 4 DIDX=0

详见 `docs/debug/rx_ring4_problem.md`

**核心事实**: 固件从不向 RX Ring 4 写入任何帧 (DIDX 始终=0)。
Ring 6 (MCU 事件) 正常工作，扫描 160 BSS 走 Ring 6 内部路径。
TX 已确认工作 (S31 AR9271 空口抓包)，问题是 Auth-2 收不到。
