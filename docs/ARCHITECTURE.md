...
**驱动状态**: 完全工作（S45 — 硬件加密 + DHCP + 互联网，重启后稳定）
...
### 3.5 RX 帧处理：802.3 快速路径 + EAPOL bypass + 硬件加密

**问题背景**：MT6639 固件启用了 `MDP_DCR0` 的 `RX_HDR_TRANS_EN`（BIT(19)），硬件自动将收到的 802.11 帧转换为 802.3 格式。驱动设置 `RX_FLAG_8023` 走 mac80211 802.3 快速路径。

**EAPOL 鸡生蛋问题（S45 修复）**：

mac80211 的 `__ieee80211_rx_handle_8023()` 要求 `fast_rx` 非 NULL，而 `fast_rx` 仅在 STA 到达 AUTHORIZED 状态后由 `ieee80211_check_fast_rx()` 设置。EAPOL Key 帧在 4-way handshake 期间到达（STA 还在 ASSOC 状态）→ `fast_rx=NULL` → mac80211 无条件丢弃 EAPOL → 无法完成 4-way handshake → 无法到达 AUTHORIZED = 死锁。

**解决方案**（`mt7927_mac.c`:`mt7927_queue_rx_skb()` EAPOL bypass 段）：

```c
// 检测 802.3 EAPOL (ethertype = ETH_P_PAE = 0x888E)
// 通过 cfg80211_rx_control_port() 直接投递给 wpa_supplicant
// 走 nl80211 control port 路径，绕过 mac80211 8023 fast path
skb->dev = dev->wifi_ndev;
skb->protocol = eth_type_trans(skb, dev->wifi_ndev);
skb->pkt_type = PACKET_HOST;
cfg80211_rx_control_port(dev->wifi_ndev, skb, true, -1);
dev_kfree_skb(skb);  // cfg80211 不消费 skb
```

关键细节：
- 现代 wpa_supplicant 使用 nl80211 control port 接收 EAPOL，**不是** raw socket
- `netif_receive_skb()` 走 raw socket 路径 → wpa_supplicant 收不到
- `cfg80211_rx_control_port()` 走 nl80211 路径 → wpa_supplicant 正确收到
- 硬件头翻译的 DA 使用 EFUSE permaddr（非接口随机化 MAC），但 cfg80211 路径不依赖 DA 匹配
- AUTHORIZED 后数据帧正常走 RX_FLAG_8023 快速路径（`fast_rx` 已设置）
...
- S45: EAPOL bypass via cfg80211_rx_control_port（解决 8023 fast path 鸡生蛋死锁）
...
| 文件 | 行数 | 核心内容 |
|------|------|---------|
| `src/mt7927_pci.c` | ~4920 | 初始化序列、MCU 通信、mac80211 回调、连接流程 |
| `src/mt7927_pci.h` | ~2170 | 全部寄存器定义、CONNAC3 TXD/RXD 位域、数据结构 |
| `src/mt7927_mac.c` | ~1600 | TXD 构建（CT+SF 两路）、RXD 解析、RX 帧分发、EAPOL bypass |
| `src/mt7927_dma.c` | ~1000 | IRQ 处理、NAPI poll、TX/RX DMA 操作 |
...
*文档更新时间：2026-02-24*
*驱动版本：Session 45（EAPOL bypass + 硬件加密 + 互联网连接，重启后稳定）*
